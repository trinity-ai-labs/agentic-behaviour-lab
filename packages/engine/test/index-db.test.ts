...
describe('TrialIndex', () => {
  ...
  beforeEach(() => {
    ...
    TestLive = Layer.mergeAll(
      artifactStoreLive,
      TrialIndexLive.pipe(Layer.provide(artifactStoreLive)),
  ...
  it.effect('insertTrial then cellSummaries reflects the outcome counts', () =>
    Effect.gen(function* () {
      ...
      yield* index.insertTrial(
        makeTrial({
          trialId: 't2',
          verdict: { outcome: 'fail', gradedBy: 'mechanical', detail: {} },
        }),
      );
  ...
  it.effect('failRate is null with no pass/fail trials yet', () =>
    Effect.gen(function* () {
      ...
      yield* index.insertTrial(
        makeTrial({
          trialId: 't1',
          verdict: { outcome: 'inconclusive', gradedBy: 'mechanical', detail: {} },
        }),
      );
      const summaries = yield* index.cellSummaries();
