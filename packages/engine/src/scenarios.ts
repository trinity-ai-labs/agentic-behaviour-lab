...
export class ScenarioRepo extends Context.Tag('@abl/engine/ScenarioRepo')<
  ScenarioRepo,
  ScenarioRepoShape
>() {}

const contentHash = (bytes: Uint8Array | string): string =>
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const loadFromDir = (
        scenarioId: string,
        dir: string,
      ): Effect.Effect<LoadedScenario, ScenarioInvalid> =>
        Effect.gen(function* () {
