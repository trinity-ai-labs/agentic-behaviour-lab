    ...
    Effect.gen(function* () {
      ...
      const wrap =
        ...
        <A, E>(effect: Effect.Effect<A, E>): Effect.Effect<A, StoreError> =>
          effect.pipe(Effect.mapError((cause) => new StoreError({ operation, path: filePath, cause })))

      const writeJson = <A>(operation: string, file: string, codec: Schema.Schema<A, string>, value: A) =>
        wrap(
          operation,
          file,
        )(
      ...
      const readJson = <A>(operation: string, file: string, codec: Schema.Schema<A, string>) =>
        wrap(operation, file)(fs.readFileString(file).pipe(Effect.flatMap(Schema.decodeUnknown(codec))))

      const runPath = (runId: string) => path.join(runsRoot, runId, "run.json")
      const trialPathFor = (runId: string, trialId: string) => path.join(runsRoot, runId, trialId, "trial.json")
      const workspacePath = (runId: string, trialId: string) => path.join(root, "workspaces", runId, trialId)
      ...
      const writeTrial: ArtifactStoreShape["writeTrial"] = (trial) =>
        writeJson("writeTrial", trialPathFor(trial.runId, trial.trialId), TrialRecordFromJson, trial)
        ...
        return wrap(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.filter(
              entries,
              (entry) => fs.exists(path.join(dir, entry, "trial.json")).pipe(Effect.orElseSucceed(() => false)),
              { concurrency: 16 },
            )
          }),
        )
          ...
          (runId) =>
            listTrialIds(runId).pipe(
              Effect.flatMap((trialIds) =>
                Effect.forEach(trialIds, (trialId) => readTrial(runId, trialId), { concurrency: 16 }),
              ),
            ),
          { concurrency: 4 },
        )
      ...
      const writeArtifact: ArtifactStoreShape["writeArtifact"] = (runId, trialId, relPath, content) => {
      ...
      const trialDir: ArtifactStoreShape["trialDir"] = (runId, trialId) => path.join(runsRoot, runId, trialId)

      const makeWorkspace: ArtifactStoreShape["makeWorkspace"] = (runId, trialId) => {
        const dir = workspacePath(runId, trialId)
        return wrap("makeWorkspace", dir)(fs.makeDirectory(dir, { recursive: true }).pipe(Effect.as(dir)))
      }
