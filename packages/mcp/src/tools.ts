...
const runTool = <A>(runtime: LabRuntime, effect: Effect.Effect<A, unknown, LabServices>): Promise<ToolResult> =>
  runtime
    .runPromiseExit(effect)
    .then((exit) => (Exit.isSuccess(exit) ? jsonResult(exit.value) : errorResult(describeCause(exit.cause))))
