...
describe('@abl/mcp server', () => {
  ...
  it('runs a batch fire-and-poll and serves results and trial details', async () => {
    ...
    const results = await callJson<Array<CellSummary>>('lab_results', {
      scenarioId: 'mcp-min',
      runId,
    });
    expect(results).toHaveLength(2);
    const byModel = new Map(results.map((cell) => [cell.modelId, cell]));
    expect(byModel.get('stub-complete')).toMatchObject({
      trials: 1,
      pass: 1,
      fail: 0,
      failRate: 0,
    ...
    const indexed = await callJson<Array<CellSummary>>('lab_results', {
