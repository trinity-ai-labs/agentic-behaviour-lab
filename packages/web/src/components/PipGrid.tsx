...
export const PipGrid = (props: PipGridProps) => {
  ...
  return (
    <div class={styles.grid} data-size={props.size ?? "md"} role="img" aria-label={pipGridAriaLabel(props)}>
      ...
      <Index each={Array.from({ length: pending() })}>{(_, i) => <VerdictPip outcome="pending" position={i + 1} />}</Index>
    </div>
  )
}
...
const pipGridAriaLabel = (props: PipGridProps): string => {
  ...
  const parts = VERDICT_ORDER.map((o) => `${outcomeCount(props, o)} ${VERDICT_META[o].label.toLowerCase()}`)
...
const VerdictPip = (props: { outcome: VerdictOutcome | "pending"; position: number }) => {
  ...
  return (
    <span
      ...
      title={isPending() ? `Trial ${props.position}: pending` : `Trial ${props.position}: ${meta()!.label}`}
    >
