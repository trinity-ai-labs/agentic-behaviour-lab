...
export type { CellProgress, CellSummary, ExecutionShape, RunConfig, RunDetail, RunRecord, RunStarted, ScenarioDefinition, TrialDetail, TrialRecord }
...
const request = async <T>(path: string, init?: RequestInit): Promise<T> => {
  ...
  if (!res.ok) {
    ...
    const message = typeof body?.message === "string" ? body.message : `${res.status} ${res.statusText}`
...
export const api = {
  ...
  trials: {
    get: (trialId: string): Promise<TrialDetail> => request(`/trials/${encodeURIComponent(trialId)}`),
  },
}
