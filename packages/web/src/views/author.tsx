...
const fieldLabelStyle: JSX.CSSProperties = { display: "flex", "flex-direction": "column", gap: "0.375rem" }
...
export const AuthorView: Component = () => {
  ...
  const [saved, setSaved] = createSignal<{ readonly scenarioId: string; readonly title: string } | undefined>(
    undefined,
  )
  ...
  const canSave = createMemo(
    () => (status() === "drafted" || status() === "saving") && scenarioId().trim().length > 0 && acceptedFiles().length > 0,
  )
  ...
  const handleDraft = async (): Promise<void> => {
    ...
    try {
      ...
      const draft = await draftScenario(
        trimmedNotes.length > 0 ? { description: description(), notes: trimmedNotes } : { description: description() },
      )
  ...
  const toggleAccepted = (path: string): void => {
    setFiles((current) => current.map((file) => (file.path === path ? { ...file, accepted: !file.accepted } : file)))
  }
  ...
  return (
    <div style={panelStyle}>
      ...
      <p style={{ color: "var(--muted)" }}>
        Describe what you want tested, in plain language. You'll get a drafted scenario back to review before
        anything is saved.
      </p>
      ...
      <button style={buttonStyle(!canDraft())} disabled={!canDraft()} onClick={() => void handleDraft()}>
        {status() === "drafting" ? "Drafting…" : "Draft scenario"}
      </button>

      <Show when={errorMessage()}>{(message) => <div style={errorBannerStyle}>{message()}</div>}</Show>
      ...
      <Show when={status() === "drafted" || status() === "saving" || status() === "saved"}>
            ...
            {(file) => (
              <div style={cardStyle}>
                <label style={cardHeaderStyle}>
                  <input type="checkbox" checked={file.accepted} onChange={() => toggleAccepted(file.path)} />
                  <span>{file.path}</span>
                </label>
        ...
        <button style={buttonStyle(!canSave())} disabled={!canSave()} onClick={() => void handleSave()}>
