...
// Quote-agnostic: Prettier normalizes CSS attribute-selector quotes to single,
// so match either `"` or `'` around the theme/scheme values.
...
for (const file of themeFiles) {
  ...
  if (schemes.join(',') !== 'dark,light') {
    problems.push(
      `${file}: expected one "light" and one "dark" block, found [${schemes.join(', ')}]`,
    );
  }
